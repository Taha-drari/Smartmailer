<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Edit Email Template</h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="max-w-3xl mx-auto">
            ...
            <form action="{{ route('email-templates.update', $emailTemplate) }}" method="POST" class="space-y-6 bg-white shadow-sm rounded-lg p-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="10" required
                        class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('body') border-red-300 @enderror">{{ old('body', $emailTemplate->body) }}</textarea>
                    @error('body')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
