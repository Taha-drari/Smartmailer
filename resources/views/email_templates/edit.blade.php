<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-white sm:text-3xl sm:truncate">Edit Email Template</h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="max-w-3xl mx-auto">
            ...
            <form action="{{ route('email-templates.update', $emailTemplate) }}" method="POST" class="space-y-8 bg-[#18181b] shadow-xl rounded-2xl p-8">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="10" required
                        class="mt-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow rounded-xl border-[#232326] bg-[#232326] text-white text-base px-4 py-3 @error('body') border-red-400 @enderror">{{ old('body', $emailTemplate->body) }}</textarea>
                    @error('body')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
